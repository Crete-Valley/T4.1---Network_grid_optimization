...
# Demo
`docker compose up` on `docker/compose.yaml` will serve the website on port 3000 ( change port in package.json script and compose.yaml if necessary)<br>
...
# Development
Use `compose-dev.yaml` for dps-server development. Hot reload is not activated, but the uvicorn flags of the dps-server<br>
service command can be changed to do so: `--reload`.
This require to recurse submodules on pull.